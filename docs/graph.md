...
that `low_mgdl` cannot be set below 50, and the app says so at startup
if it is.
...
- **Except in front of the newest point.** One response carries both the
  ...
  `graphData`'s newest entry trails `glucoseMeasurement`, by 18 minutes
...
- **The time axis is ruled too**, with a short stub every hour and a longer
...
- **They thin out rather than crowd.** A grown axis fits more lines into
  ...
  label. The ruling itself never thins, and you are still counting
  fifties.
